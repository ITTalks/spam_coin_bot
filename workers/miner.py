...
class Miner(ApiBase):
    def __init__(self, token: str):
        ...
        self.check_dict = {"3": 50000, "4": 100000, "5": 200000, "6": 500000,
                           "7": 800000,
                           "8": 1000000, "9": 1500000,
                           "10": 2000000}
    ...
    @staticmethod
    async def get_hash():
        # TODO: вот это закинуть в солве блок... чтобы не крашился
    ...
    async def solve_block(self, start, end, thread_name):
        ...
        for nonce in reversed(range(start, end)):
            ...
            if i == self.check_dict[zero_value_start]:
                ...
                if hash_ != hash_first:
                    logging.info(
                        f" thread-{thread_name}: Changed hash to {hash_}")
                    nonce = start
                    hash_first = hash_
            ...
            if sha.hexdigest().startswith("0" * int(zero_value_start)):
                r = await self.api_request("mining.send",
                                           params={"nonce": nonce})
                logging.info(
